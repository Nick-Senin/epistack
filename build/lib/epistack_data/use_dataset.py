...
def for_extraction_module(hf_username: str):
    """Датасет для оптимизации module_extraction_by_name"""
...
def for_naming_module(hf_username: str = None):
    """Датасет для оптимизации module_naming"""
    # Если username не указан, используем локальный датасет
    if hf_username is None:
        from datasets import load_from_disk
        local_path = os.path.join(os.path.dirname(__file__), 'epistack_optimization_local')
        if os.path.exists(local_path):
            ds = load_from_disk(local_path)['train']
            print(f"✅ Загружен локальный датасет: {len(ds)} примеров")
        else:
            raise FileNotFoundError(f"Локальный датасет не найден: {local_path}")
    else:
        ds = load_epistack_dataset(hf_username, split='train')
